...
Param(
   ...
   [Parameter(Mandatory=$True,Position=1,HelpMessage="nome do modulo usado para compor a pasta de publicacao")]
   [string]$appname,
...
function ZIP-AppSVC($SourceFolder,$PubRootFolder,$Sistema,$pNomeZip,$KeepBINFiles)
{
    ...
    try 
    {
        ...
        if(Test-Path $PubRootFolder)
        {
            ...
            Remove-Item $FolderBINExclusao -Exclude $KeepBINFiles 
            #Gerar o arquivo zip 
            $ZipFile = $PubRootFolder + '\' + $pNomeZip + '.zip'
            Write-Host 'Gerando o arquivo ZIP' $ZipFile
...
{
    ZIP-AppSVC $sourcerootfolder $publishrootfolder $appname $nomezip $keepbinfiles
}
